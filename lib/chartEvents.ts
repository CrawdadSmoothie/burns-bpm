...
const PEAK_MIN_BPM = 90;

// Meltdown: BPM must be ≥125 AND have risen by at least this much over the
// previous ~5 seconds. Filters out small oscillations around the threshold.
const MELTDOWN_VELOCITY_BPM = 14;
 ...
 * "meltdown" — meaningful rage spike: BPM ≥125 with rapid rise, throttled by
 *              a cooldown that requires BPM to fall below 110 before re-firing.
 * "redline"  — first crossing from < 110 to ≥ 110 (lower-priority context marker).
 */
export function extractChartEvents(points: HeartRatePoint[]): ChartEvent[] {
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    // ── Redline: first crossing from < 110 to ≥ 110 ────────────────────
    if (!redlineDone && prevBpm < 110 && p.bpm >= 110) {
      redlineDone = true;
    ...
    // ── Meltdown: ≥125 bpm with meaningful velocity ─────────────────────
    // Velocity = delta over the previous ~5 seconds (5 data points at 1 Hz)
    if (p.bpm >= 125 && meltdownResetReady) {
